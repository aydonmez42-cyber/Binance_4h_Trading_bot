...
def main():
    ...
    long_df = fetch_klines(cfg.LONG_SYMBOL, cfg.INTERVAL, cfg.DATA_START, cfg.DATA_END)
    short_df = fetch_klines(cfg.SHORT_SYMBOL, cfg.INTERVAL, cfg.DATA_START, cfg.DATA_END)
    print(f"Rows: signal={len(signal_df):,}, long={len(long_df):,}, short={len(short_df):,}")
    signal_df = add_indicators(signal_df, cfg)
    print(f"TEST 16 | Fixed {cfg.POSITION_QTY_ETH} ETH | LONG {cfg.LONG_SYMBOL} | SHORT {cfg.SHORT_SYMBOL} | RSI short < {cfg.RSI_SHORT_THRESHOLD}")
