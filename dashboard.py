...
async function refreshBistScanner(){let d=await bistScannerData();bistScannerCache=d;let st=d.status||'IDLE';let src=d.universe_source?` • Evren: ${d.universe_source}`:'';let txt=st==='SCANNING'?`BIST TÜM taraması: ${d.symbols_done||0}/${d.symbols_total||0}`:st==='READY'?`Hazır • Son 4H: ${d.last_scan_candle||'—'}${src}`:st==='ERROR'?`Hata: ${d.last_error||'Bilinmeyen hata'}`:'Bekleniyor...';document.getElementById('bistScannerStatus').textContent=txt;renderBistScanner();}
async function startBistScanner(force=false){document.getElementById('bistScannerStatus').textContent='BIST TÜM taraması başlatılıyor...';try{await fetch('/api/bist-scanner/scan?force='+(force?'1':'0'),{cache:'no-store'})}catch(e){}refreshBistScanner();}
